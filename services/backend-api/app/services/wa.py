...
def _heuristic_parse(text: str) -> ParsedIntent:
    ...
    return ParsedIntent(intent=intent, amount=amount, description=description, category_suggestion=category)


def _detect_direction(text: str) -> str:
    lowered = text.lower()
    income_keywords = ["gaji", "terima", "masuk", "bonus", "bayaran"]
    transfer_keywords = ["transfer", "kirim", "pindah"]
    if any(word in lowered for word in income_keywords):
        return "income"
    if any(word in lowered for word in transfer_keywords):
        return "transfer"
    return "expense"
